...
log_regr_stoch_mom = function(X,y, tol = 0.001, alpha = 0.9, nu = 0.1){
  ...
  while ((error > tol) && (step < max_iter)){
    ...
    w_cur = w_prev - nu * t(g) + alpha * delta_w
  ...
  return(list(w_cur, error, g, yhat, step))
